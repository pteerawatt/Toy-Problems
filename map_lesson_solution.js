...
console.log('Should explode => ', outerFunct(smallDynomite));
...
let mapArray = values.map((element, index, arr) => {
...
let namesWithNum = names.map((name, index) => {
  return `${name} - ${index}`;
});
